...
--  request-to-reserve booking system.
--
--  Design notes
--  ------------
--  * CONTENT (room names, photos, copy, price) lives in Keystatic. AVAILABILITY
--    and RESERVATIONS live here. `rooms_meta.slug` is the stable join key between
--    the two worlds.
--  * This business takes bookings on Booking.com, Airbnb, by phone, AND the
--    website at the same time. Double-bookings are the #1 risk, handled for free
--    with periodic iCal sync (NOT real-time) — which is exactly why bookings stay
--    request-to-reserve: owner confirmation is the safety net against sync lag.
